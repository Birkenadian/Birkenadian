---
name: review
description: Compare the current build against the spec at specs/writing.md, requirement by requirement, and either pass it or produce a specific fix list for /build. Use when the user runs /review or asks to check the build against the spec.
---
...
- Read `specs/writing.md` in full. If it doesn't exist, stop and tell the user to run `/spec` first.
- Find the deliverable the build produced (the spec usually says where it lives; otherwise check `drafts/` and the most recent `/build` coverage report). If no deliverable exists, stop and tell the user to run `/build` first.
...
**If anything fails:** declare the build **FAILED** and write the fix list to `specs/review.md` so `/build` can pick it up. For each failure include:

1. **The exact spec item it fails** — quoted, with its number/section from `specs/writing.md`.
...
End by telling the user the build failed N of M items, that the fix list is at `specs/review.md`, and to run `/build` again to address it. Do not apply the fixes yourself — fixing is `/build`'s job.

When a build later passes after a previous failure, note in your summary that prior fix list items were resolved, and say `specs/review.md` is now stale.
